...
def safe_untar(file_like_obj, username, uuid):
    ...
    try:
        ...
        score = model_api(tmp_file)
        #score = f'{score:.3f}'
        write_to_db(username, uuid, score)
        tar.close()
        
        logger.info('Done!')
        
        return {'status': 'good', 'description': 'UploadSuccess', 'score':score}
...
def write_to_db(user, uuid, score):
    ...
    if r['exists']:
        ...
        requests.post(f'http://mongoapi:3386/internal/user/update?username={user}&score={score}')
    else:
